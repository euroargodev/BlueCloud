...
def generate_plots(m, ds, var_name_ds):
    ...
    P.save_BlueCloud('figures/vertical_struc_EX' + file_ext + '.png')
    # plot profiles by quantile
    P.vertical_structure_comp(q_variable=var_name_ds + '_Q', plot_q='all', xlabel='Temperature (°C)', ylim=[-1000, 0])
    P.save_BlueCloud('figures/vertical_struc_comp_EX' + file_ext + '.png')
    # spacial distribution
    P.spatial_distribution(time_slice='most_freq_label')
    P.save_BlueCloud('figures/spatial_distr_freq_EX' + file_ext + '.png')
    # robustness
    ...
    P.pie_classes()
    P.save_BlueCloud('figures/pie_chart_EX' + file_ext + '.png')
    # temporal distribution (monthly)
    P.temporal_distribution(time_bins='month')
    P.save_BlueCloud('figures/temporal_distr_months_EX' + file_ext + '.png')
    # temporal distribution (seasonally)
    P.temporal_distribution(time_bins='season')
    P.save_BlueCloud('figures/temporal_distr_season_EX' + file_ext + '.png')
    # save data
    # TODO fix export bug
    # ds.to_netcdf('datasets/mediterranean_2018_temp_predicted_dataset' + file_ext + '.nc') error on the export (UnicodeDecodeError)
