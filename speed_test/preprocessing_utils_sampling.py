...
def fit_pca(x, n_comp, var_name, backend):
    if backend == 'sk':
...
def preprocessing_allin(path, scaling, backend, multiple, var_name, n_comp_pca, spacial_domain='glob'):
    ...
    x = x.transpose()
    if scaling:
        x, _ = standard_scaling(x, backend, var_name)
        x, _ = fit_pca(x=x, n_comp=n_comp_pca, var_name=var_name + '_scaled', backend=backend)
    else:
        x, _ = fit_pca(x=x, n_comp=n_comp_pca, var_name=var_name, backend=backend)
    return x
